...
describe("App routing", () => {
  ...
  it("renders the landing page content and navigates through landing CTAs", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "AI Issue Scope Guard" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/檢查 AI 產生的 PR 是否守住 GitHub Issue 任務範圍/),
    ).toBeInTheDocument();
  ...
  it("renders the rule explanation page content", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Rule-based Analyzer 檢查規則" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/MVP 是 rule-based 且 local-only/),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "範圍符合度分數邏輯" }),
    ).toBeInTheDocument();
    expect(screen.getByText(/Base score 從 100 分開始/)).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "扣分規則" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("90-100 = 低風險 / 可進入審查"),
    ).toBeInTheDocument();
  ...
  it("loads demo data, displays analysis results, copies the PR comment, and clears state", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "複製 PR 評語" }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByPlaceholderText(
        "貼上 PR Summary，例如實作內容、修改原因、驗證方式。",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText("建議每行貼上一個 changed file，讓檔案範圍更容易檢查。"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("請貼上實際指令結果；如果沒有執行，請明確寫出原因。"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "請明確寫出是否有 dependency 變更，避免 reviewer 需要猜測。",
      ),
    ).toBeInTheDocument();

    await fireEvent.click(screen.getByRole("button", { name: "良好 PR 範例" }));
    expect(
      (screen.getByLabelText("Issue 任務範圍") as HTMLTextAreaElement).value,
    ).toContain("Issue #12");
    ...
    expect(
      screen.getByText("部分檢查項目需要進一步確認。"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
    ...
    await fireEvent.click(
      screen.getByRole("button", { name: "高風險 PR 範例" }),
    );
    expect(
      (screen.getByLabelText("變更檔案") as HTMLTextAreaElement).value,
    ).toContain(".env.local");
    ...
    const highRiskGroupTitles = within(
      screen.getByTestId("check-results-groups"),
    )
      .getAllByRole("heading", { level: 3 })
      .map((heading) => heading.textContent);
    ...
    expect(
      screen.getByText(
        "要求 Codex 移除 out-of-scope changes，或拆成新的 Issue / PR。",
      ),
    ).toBeInTheDocument();
  ...
  it("switches PR comment modes and copies the selected comment", async () => {
    ...
    expect(screen.getByRole("button", { name: "簡短版" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    expect(screen.getByRole("button", { name: "詳細版" })).toHaveAttribute(
      "aria-pressed",
      "false",
    );
